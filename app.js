...
<body>
    ...
    <script>
        ...
        function switchAdminTab(tab) {
            ['stats','accounts','loans','cards','support','tickets-log','staff','locks','settings','bank-log'].forEach(t => {
        ...
        async function loadMyCards() {
            ...
            container.innerHTML = cards.map(c => {
                ...
                const realCard = c.status === 'approved' && c.cardNumber ? \`
                    <div style="background:linear-gradient(135deg,#1a3a6b,#0f2044); border:1px solid #3b82f6; border-radius:16px; padding:20px 22px; margin-top:12px; font-family:monospace; position:relative; overflow:hidden; min-width:280px; max-width:380px;">
                        <div style="position:absolute; top:-20px; right:-20px; width:100px; height:100px; background:rgba(59,130,246,0.08); border-radius:50%;"></div>
                        <div style="position:absolute; bottom:-30px; left:-10px; width:130px; height:130px; background:rgba(59,130,246,0.06); border-radius:50%;"></div>
                        <p style="font-size:0.7rem; color:#60a5fa; letter-spacing:2px; margin-bottom:14px;">NORV BANK</p>
                        <p style="font-size:1.25rem; letter-spacing:3px; color:#e2e8f0; margin-bottom:14px;">\${c.cardNumber || '#### #### #### ####'}</p>
                        <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap; gap:8px;">
                            <div>
                                <p style="font-size:0.65rem; color:#60a5fa; letter-spacing:1px;">CARD HOLDER</p>
                                <p style="font-size:0.9rem; color:#e2e8f0; text-transform:uppercase;">\${c.cardHolderName || '---'}</p>
                            </div>
                            <div>
                                <p style="font-size:0.65rem; color:#60a5fa; letter-spacing:1px;">EXPIRES</p>
                                <p style="font-size:0.9rem; color:\${isExpired ? '#fca5a5' : '#e2e8f0'};">\${expiryStr}</p>
                            </div>
                            <div>
                                <p style="font-size:0.65rem; color:#60a5fa; letter-spacing:1px;">CVV</p>
